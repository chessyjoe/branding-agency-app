...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Enhance the prompt for business card generation
      ...
      // Generate the business card based on the selected model
      if (model === "dall-e-3" && openaiApiKey) {
        const response = await fetch("https://api.openai.com/v1/images/generations", {
          ...
          body: JSON.stringify({
            ...
            n: 1,
            size: "1792x1024", // Business card format
        ...
        const submitResponse = await fetch(fullUrl, {
          ...
          body: JSON.stringify({
            prompt: refinedPrompt,
            aspect_ratio: "16:9", // Business card format
          }),
        })
      ...
      try {
        await fetch(`${request.nextUrl.origin}/api/auto-save-generation`, {
          ...
          body: JSON.stringify({
            ...
            eventDetails,
            aspectRatio: "16:9",
            tags: ["business-card", "professional", "print"],
          }),
        })
      } catch (autoSaveError) {
      ...
      return NextResponse.json({
        ...
        refinedPrompt,
        resultType: "image",
        message: "Business card generated and auto-saved successfully",
      })
    } catch (error) {
